...
function getLiveStateNews() {
  ...
  var show_states_lang = ['gujarati','hindi','english'] ;
  var parentURL = document.referrer;
  // var parentURL = 'https://www.etvbharat.com/bengali/kerala';
  var temp = parentURL.split("/");
  // var lang = temp[4];
  ...
  national = "hindi";
  switch (national) {
    case 'hindi': $('#heading-tag').html('विधानसभा चुनाव 2022 परिणाम लाइव'); hindiAdd(); break;
    case 'telugu': $('#heading-tag').html('అసెంబ్లీ ఎలక్షన్స్ 2022 రిజల్ట్ లైవ్'); break;
    case 'gujarati': $('#heading-tag').html('વિધાનસભા ચૂંટણી 2022નું પરિણામ લાઈવ'); gujaratAdd(); break;
    case 'assamese': $('#heading-tag').html('বিধানসভা নিৰ্বাচন 2022 ৰ ফলাফল লাইভ'); break;
    case 'urdu': $('#heading-tag').html('اسمبلی انتخابات 2022 نتیجہ لائیو,'); break;
    case 'kannada': $('#heading-tag').html('ವಿಧಾನಸಭಾ ಚುನಾವಣೆ 2022 ಫಲಿತಾಂಶ ನೇರಪ್ರಸಾರ'); break;
    case 'malayalam': $('#heading-tag').html('നിയമസഭ തെരഞ്ഞെടുപ്പ്  ഫലം 2022 തത്സമയം '); break;
    case 'marathi': $('#heading-tag').html('विधानसभा निवडणूक २०२२ निकाल लाईव्ह'); break;
    case 'oriya': $('#heading-tag').html('ବିଧାନସଭା ନିର୍ବାଚନ 2022 ଫଳାଫଳ ଲାଇଭ'); break;
    case 'punjabi': $('#heading-tag').html('Punjab Election 2022 Results Live'); break;
    case 'tamil': $('#heading-tag').html('சட்டப்பேரவை தேர்தல் 2022 முடிவுகள் நேரலை'); break;
    case 'bengali': $('#heading-tag').html('বিধানসভা নির্বাচন ২০২২ ফলাফল লাইভ'); break;
    case 'english': $('#heading-tag').html('Assembly Elections 2022 Result Live'); englishAdd(); break;
  }
  if (show_states_lang.includes(national)) {
    national == 'gujarati' ? $("#selectStateShow").hide() : $("#selectStateShow").show();
    $("#other_state_show").show();
  } else {
    $("#selectStateShow").hide();
    $("#other_state_show").hide();
    $("#state-adds").hide();
  }
...
function fiveStateTally(CombineTable, theadh, state) {
  ...
  for (i = 0; i < theadh.length; i++) {
    ...
    t = t + "<td class='tdata2' style='font-weight: 700;color:black;'><div width='100'>" + theadh[i] + "</div></td>";
  }
  CombineTable = "<tr><td class='tdata2' style='font-weight: 600;color:black;border-bottom:1px solid #ddd;'><div>Party</div></td>" + t + "</tr><tr>" + "<td class='' style='font-weight: 600;'>Lead</td>" + CombineTable[0] + "</tr><tr>" + "<td class='' style='font-weight: 600;'>Won</td>" + CombineTable[1] + "</tr><tr>" + "<td class='' style='font-weight: 600;'>Total</td>" + CombineTable[2] + "</tr>";

  // CombineTable = "<tr><th class='lead-won' style='background: #B80403;'><div> </div></th>" + t + "</tr><tr>" + "<td class='lead-won' style='font-weight: 700;'>Lead/won</td>" + CombineTable + "</tr>";border-right:1px solid #bb454f;
  if (state === "UP") {
    ...
    for (var j = 0; j < arr.length; j++) {
      ...
      for (i = 0; i < response.totalresults.length; i++) {
        if (response.totalresults[i].state_id == arr[j] && response.totalresults[i].party_name) {
          ...
          var t_lead = "<td class='td-style'>" + response.totalresults[i].lead + "</td>";
          lead_row = lead_row + t_lead;
          
          var t_won = "<td class='td-style'>" + response.totalresults[i].won + "</td>";
          won_row = won_row +t_won;
         
          var t_total = "<td class='td-style' style='font-weight: 700;'>" + response.totalresults[i].total + "</td>";
          total_row = total_row +t_total;
      ...
      CombineTable.push(lead_row,won_row,total_row)
...
function getPrevYear() {
  ...
  if (state_el == "gujarat_select") {
    var frame1 ='<iframe title="GUJARAT ASSEMBLY ELECTION 2017" aria-label="Map"id="datawrapper-chart-dryu7" src="https://datawrapper.dwcdn.net/dryu7/1/" scrolling="no" frameborder="0" style="border: none;" width="100%" height="760" data-external="1"></iframe>';
    $("#iframe-state").html(frame1);
  } else if (state_el == "himachal_select") {
    var frame1 =
      '<iframe title="HIMACHAL PRADESH ASSEMBLY ELECTION 2017" aria-label="Map" id="datawrapper-chart-AoUdi" src="https://datawrapper.dwcdn.net/AoUdi/1/" scrolling="no" frameborder="0" style="border: none;" width="100%" height="765" data-external="1"></iframe>';
    $("#iframe-state").html(frame1);
  } 
...
function getDistrictResult() {
  ...
  if (state == "gujarat_select") {
    ...
    $.getJSON(url, function (successdata) {
      CombinedHTMLTable = "";
      var thead = "";
      thead = "<tr class='dist_thead_fix'>" + "<th class='table-h'></th>" + upHead() + "</tr>";
...
function gujaratAdd() {
  ...
  $("#web-ad1")
    .html(`<div id='div-gpt-ad-1669112784013-0' style='min-width: 300px; min-height: 250px;'>
            <script>
              googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112784013-0'); });
            </script>
          </div>`);
  $("#web-ad2")
    .html(`<div id='div-gpt-ad-1669112784013-1' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112784013-1'); });
    </script>
  </div>`);
  $("#web-ad3")
    .html(`<div id='div-gpt-ad-1669112784013-2' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112784013-2'); });
    </script>
  </div>`);
  $("#mob-add")
    .html(`<div id='div-gpt-ad-1669112784013-3' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112784013-3'); });
    </script>
  </div>`);
}
...
function hindiAdd() {
  ...
  // $("#state-img").html(upImg);
  $("#web-ad1")
    .html(`<div id='div-gpt-ad-1669113020014-0' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669113020014-0'); });
    </script>
  </div>`);
  $("#web-ad2")
    .html(`<div id='div-gpt-ad-1669113020014-1' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669113020014-1'); });
    </script>
  </div>`);
  $("#web-ad3")
    .html(`<div id='div-gpt-ad-1669113020014-2' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669113020014-2'); });
    </script>
  </div>`);
  $("#mob-add")
    .html(`<div id='div-gpt-ad-1669113020014-3' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669113020014-3'); });
    </script>
  </div>`);
...
function englishAdd() {
  ...
  // $("#state-img").html(upImg);
  $("#web-ad1")
    .html(`<div id='div-gpt-ad-1669112461602-0' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112461602-0'); });
    </script>
  </div>`);
  $("#web-ad2")
    .html(`<div id='div-gpt-ad-1669112461602-1' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112461602-1'); });
    </script>
  </div>`);
  $("#web-ad3")
    .html(`<div id='div-gpt-ad-1669112461602-2' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112461602-2'); });
    </script>
  </div>`);
  $("#mob-add")
    .html(`<div id='div-gpt-ad-1669112461602-3' style='min-width: 300px; min-height: 250px;'>
    <script>
      googletag.cmd.push(function() { googletag.display('div-gpt-ad-1669112461602-3'); });
    </script>
  </div>`);
}
// function getPcConst() {
